...
function search_perform($terms, $table, $field,$join) {
	global $conn;
	...
	foreach($terms_db as $term_db){
		$parts[] = "$field LIKE '$term_db'";
	}
	$parts = implode(' AND ', $parts);

	$sql = "SELECT $table.*,$join.name AS users_name FROM $table LEFT JOIN $join ON $join.id = $table.user_id WHERE $parts";
